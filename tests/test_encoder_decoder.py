...
def test_encode_empty_message():
    ...
    assert "Message protocol.TMProposeSet is missing required fields" in str(excinfo.value)
...
def test_decode_unknown_packet():
    ...
    encoded_packet = packet_pb2.Packet(
        data=b'\x00\x00\x00\x0B' + message_type.to_bytes(2, "big") + b'\x08\x01\x12\x06\x08\x02\x10\x00')
...
def test_decode_different_message_type():
    ...
    header = (
            (len(serialized_message).to_bytes(4, "big"))
            + message_type.to_bytes(2, "big")
    )
    encoded_packet = packet_pb2.Packet(data=header + serialized_message, from_port=60001, to_port=60002)

    # Perform decoding
    decoded_message, decoded_message_type = PacketEncoderDecoder.decode_packet(encoded_packet)
...
def test_signature_correct_with_propose():
    ...
    assert message.signature.hex() == "3045022100ce81f7d0bd5195ff27f7ec736beaa5cbd2e47bfd50b9fdda26036f4569ff0c67022003e6bf75081eb0cbf63adfa07b77218fa3c14a6904819a505a91da5c23bc02ab"
...
def test_signature_error():
    ...
    with pytest.raises(NotImplementedError) as excinfo:
        PacketEncoderDecoder.sign_message(message, bs58_key.hex())
    assert "No signing method implemented for <class 'protos.ripple_pb2.TMTransaction'>" in str(excinfo.value)
