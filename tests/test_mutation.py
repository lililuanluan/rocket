...
def test_mutation_propose():
    ...
    strategy.public_to_private_key_map = {
...
# Encode the message into a packet
    ...
    packet: Packet = packet_pb2.Packet(data=encoded_data, from_port=60000, to_port=60001)

    # Mutate the packet data
    mutated_data, action = strategy.handle_packet(packet)

    # Create a new packet with mutated data
    mutated_packet = packet_pb2.Packet(data=mutated_data, from_port=60000, to_port=60001)

    # Decode the mutated packet
    mutated_message, mutated_message_type = PacketEncoderDecoder.decode_packet(mutated_packet)
...
def test_no_mutation_not_propose():
    ...
    packet: Packet = packet_pb2.Packet(data=encoded_data, from_port=60001, to_port=60000)
...
def test_mutation_propose_correct_signature_change():
    ...
    pub_key = base58.b58decode("n9Mdv672BQY4dvLfmnqSSme4vW1obGu4a9eNL5PQNqDfTk3qyYHd", alphabet=base58.RIPPLE_ALPHABET)
