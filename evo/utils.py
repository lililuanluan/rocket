...
def aggregate_logs(log_dir=None):
    ...
    sub_dirs.sort(key=lambda d: (int(d.name.split("G")[1].split("T")[0]), int(d.name.split("T")[1])))
    for sub_dir in sub_dirs:
        ...
        if fitness_json_path.exists():
            with open(fitness_json_path, "r") as f:
                ...
                fitness_func = fitness_data.get("fitness", "mean_validation_time") or "mean_validation_time"
