...
from evaluate import evaluate_log, EvaluationResult
import random
import numpy as np
import csv
...
def setup_deap_types():
    ...
    creator.create("Individual", list, fitness=creator.FitnessMax, 
                   log_dir=None, evaluation_result=None)
...
def run_rocket(log_dir, max_iteration, max_ledger_seq, seed, encoding):
    ...
    cmd = [
        py, "-m", "rocket_controller", "EvoDelayStrategy",
        "--config", str(strategy_yaml),
        "--network_config", str(CUR_DIR / "network.yaml"),
        "--log-dir", log_dir,
        "--max-iteration", str(max_iteration),
        "--max-ledger-seq", str(max_ledger_seq),
    ]
...
def evaluate_individual(individual, generation, individual_id):
    ...
    if FITNESS_FUNCTION == 'time':
        fitness = eval_result.mean_validation_time if eval_result.mean_validation_time else 0.0
    elif FITNESS_FUNCTION == 'proposal':
        fitness = eval_result.propose_set_count
    else:
        fitness = 0.0
...
def create_individual():
    """创建随机个体的工厂函数"""
    encoding = [random.randint(ENCODING_MIN, ENCODING_MAX) for _ in range(ENCODING_LENGTH)]
...
def init_csv_file(output_dir):
    ...
    with open(CSV_FILE_PATH, 'w', newline='') as csvfile:
        fieldnames = ['generation', 'individual_id', 'fitness_type', 'fitness', 
                      'mean_validation_time', 'propose_set_count', 'total_failures']
...
def write_individual_to_csv(generation, individual_id, fitness, eval_result):
    ...
    with open(CSV_FILE_PATH, 'a', newline='') as csvfile:
        ...
        mean_time_formatted = round(eval_result.mean_validation_time, 3) if eval_result.mean_validation_time else 0.0
    ...
    print(f"  ✓ Written to CSV: G{generation}T{individual_id}, fitness={fitness_formatted:.3f}")
...
def main(config):
    ...
    def crossover_and_round(ind1, ind2):
        """SBX 交叉后转换为整数"""
        tools.cxSimulatedBinaryBounded(ind1, ind2, eta=3.0, 
                                       low=ENCODING_MIN, up=ENCODING_MAX)
        # 转换为整数
